  ...
  return (
    <Modal title={tipo === 'documento' ? 'Partilhar Documento' : 'Partilhar Pasta'} onClose={handleClose}>
      <div style={s.body}>
        ...
        {!token ? (
          <div style={s.empty}>
            ...
            <div style={s.emptyTitle}>Sem link de partilha</div>
            <div style={s.emptySub}>
              Gera um link único para partilhar {tipo === 'documento' ? 'este ficheiro' : 'esta pasta'} com colegas.
              Qualquer pessoa com o link pode aceder sem precisar de login.
            </div>
          </div>
        ) : (
          <div style={s.linkBox}>
            <div style={s.linkLabel}>Link de partilha</div>
            <div style={s.linkRow}>
            ...
            <div style={s.linkHint}>
              {tipo === 'pasta'
                ? '👥 Colegas podem ver e descarregar ficheiros desta pasta'
                : '👤 Colegas podem ver e descarregar este ficheiro'}
            </div>
          </div>
        )}
        ...
        {token && (
          <div style={s.warn}>
            <strong>Atenção:</strong> Revogar o link impossibilita imediatamente o acesso de quem o tinha.
          </div>
        )}
      </div>
      ...
      <Modal.Footer>
        ...
        {token
          ? <Btn variant="danger" onClick={revogar} disabled={loading}>{loading ? 'A revogar…' : '🔒 Revogar link'}</Btn>
          : <Btn variant="primary" onClick={gerar} disabled={loading}>{loading ? 'A gerar…' : '🔗 Gerar link'}</Btn>
        }
      </Modal.Footer>
    </Modal>
  )
